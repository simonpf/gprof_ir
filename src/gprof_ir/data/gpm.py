...
from pansat.products.satellite.gpm import (
    ...
    l2b_gpm_cmb_b,
    l2b_gpm_cmb_c,
    l2a_clim_gprof_gpm_gmi_v08a,
    l2a_clim_gprof_gcomw1_amsr2_v08a,
)
...
class GPMGMI(ReferenceDataset):
    ...
    def find_files(
        ...
        if path is None:
            raise ValueError(
                "The GMI reference data needs the path argument."
            )

        all_files = sorted(list(path.glob("**/2A.GPM.GMI*.nc")))
        matching = []
        for path in all_files:
            parts = path.name.split(".")[4].split("-")[:2]
            date = datetime.strptime("".join(parts), "%Y%m%dS%H%M%S")
            date = to_datetime64(date)
    ...
    def process_file(
        ...
        with xr.open_dataset(path) as data:
            data = data[["surface_precip", "scan_time", "longitude", "latitude"]].compute()
        data = data.rename({
            "scan_time": "time",
        })
...
class GPMGMIClim(ReferenceDataset):
    ...
    def process_file(
        ...
        with xr.open_dataset(path) as data:
            if "surface_precip" in data:
                data = data[["surface_precip", "scan_time", "longitude", "latitude"]].compute()
            else:
                with self.pansat_product.open(path) as data:
                    data = data.rename(surface_precipitation="surface_precip")
                    data = data[["surface_precip", "scan_time", "longitude", "latitude"]].compute()
        ...
        # Need to expand scan time to full dimensions.
        time, _ = xr.broadcast(data.scan_time, data.surface_precip)
        valid_times = time.data[np.isfinite(time.data)]
        min_time = valid_times.min()
        max_time = valid_times.max()
        ...
        min_time_delta = (min_time - start_of_day).astype(time_step.dtype).astype("uint64") // time_step.astype("uint64")
        min_time_r = start_of_day + min_time_delta * time_step
        time_bins = np.arange(min_time_r - time_step, max_time + time_step / 2.0, time_step)
        lons_g, lats_g = domain[self.scale].get_lonlats()
...
gpm_amsr2_clim = GPMGMIClim("amsr2", l2a_clim_gprof_gcomw1_amsr2_v08a)
