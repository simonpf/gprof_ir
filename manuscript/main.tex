...
\citep{ncep_cpc_gpm_mergir_v1, janowiak2001real}. The dataset provides merged
geostationary IR observations from the 11-$\mu$m IR window from the global
constellation of geostationary satellites. The dataset provides observations at
half-hourly resolution covering much of the globe between $60^\circ$S and $60^\circ$N
...
the GPROF-IR retrieval. The first source is derived from the GPM Level-2
combined radar–radiometer product \citep{cmb_data}. This 2BCMB product
...
GPM constellation and is widely regarded as the most reliable global
precipitation record currently available.
...
GPROF V08 retrieval is also based on reference precipitation rates from the GPM 2BCMB,
it implements several ad-hoc solutions to address shortcomings of the reference data.
Among others, these include the use reference precipitation estimates from ground-based radar over snow-covered surfaces and the filling-in of light and frozen oceanic precipitation that is missed by the DPR using the GPROF-NN XPR retrieval (Pfreundschuh, 2026).
...
We use precipitation estimates from the IMERG Final run
\citep{gpm_imerg_final_v07}. IMERG uses a Kalman-filter-based merging scheme
\citep{huffman2019imerg_v07_atbd} to combine MW precipitation rates from GPROF
...
reanalysis dataset \citep{hersbach_er5_2020}. ERA5 is the leading global
reanalysis dataset providing continuous global coverage at hourly temporal
resolution and a spatial resolution of 0.025$^\circ$. ERA5 provides
global precipitation estimates since 1940 and therefore constitutes an
alternative to global merged precipitation products for studying the
evolution of precipitation systems. In contrast to satellite-based precipitation products, which
generally estimate precipitation rates directly from observations, ERA5 uses a
numerical weather prediction model to determine the atmospheric state that
provides the best agreement with a wide range of historical meteorological
observations.
...
precipitation retrieval. By using the publicly available SatRain data
...
precipitation estimates from the NOAA Multi-Radar Multi-Sensor (MRMS,
\citep{smith_multi-radar_2016}) dataset; the Korea data is derived from
gauge-corrected precipitation estimates provided by the Korean Meteorological
Agency. To accommodate for the testing resolution of 0.036 degree used by the
SatRain dataset, all GPROF-IR results are interpolated to the SatRain grid using
...
from the OceanRAIN dataset \citep{klepp2018oceanrain}.
Since OceanRAIN data is currently available only up until December 2018, we use
all available data from 2017 to 2018. These measurements are effectively point
sources over 30-minute windows, which we match to the closest IR retrievals.
The spatial distribution of the resulting match-ups are shown in
Fig.~\ref{fig:ocean_rain}. Since the validation period overlaps with the
...
\begin{figure}[h]
 \centerline{\includegraphics[width=37pc]{figures/figure05.pdf}}
 \caption{
...
data from the year 2022. The testing data has been derived in the same way as
the training data used for the GPROF-IR configuration but instead of data from
the years 2015--2018 it uses data from the year 2022.
...
overestimate light precipitation and underestimate high precipitation, which
typical for precipitation retrievals from passive sensors. The GPROF-IR (CMB, 1)
...
\paragraph{Effective Resolution}
 ...
 spatial coherence exceeds 1/sqrt(2), which corresponds to the scale at
 which the signal-to-noise ratio of the captured spatial variability exceeds
 one. Since the analysis of the effective resolution requires spatially continuous
 precipitation fields covering at least the resolved scales, the SatRain dataset only
 allows assessing the effective resolution over the CONUS and Korea domains.

 The spectral coherence curves for the CONUS and Korea domains are shown in Fig.
 6. Over CONUS, The GPROF-IR retrieval achieves an effective resolution of 0.6
 degree for the GPROF-IR (GMI, 5) configuration, 0.65 for the GPROF-IR (GMI, 3)
 configuration, and 0.68 for the GPROF-IR (GMI, 1) configuration. The 2BCMB-based
 retrieval, on the other hand, only achieves an effective resolution of 0.98
 degree. Over the Korea domain, the effective resolutions are much lower with
 resolutions around 1.25 degree for the GPROF-IR (GMI, 3) and GPROF-IR (GMI, 5)
 retrievals and 1.4 for the GPROF-IR (GMI, 1) and GPROF-IR (CMB) configurations.
 The ranking of the effective resolution largely mirrors that of the MAE, MSE,
 and linear correlation coefficient over CONUS.

 The GPROF-IR (GMI, 3) and GPROF-IR (GMI, 5) retrievals achieve effective
 resolutions that are about two-times finer than the GMI-based IMERG V07 PMW
 estimates over CONUS. However, the situation is reversed over the Korea domain,
 where the IMERG V07 estimates are more than two-time finer than the best
 GPROF-IR retrievals. Again, this highlights the advantage of PMW over water
 surfaces as well as the regime dependence of the accuracy of the IR-based
 retrievals.
...
Figure~\ref{fig:case_study_1} shows reference and retrieved precipitation during the landfall of
Typhoon Khanun over the Korean peninsula. The principal precipitation features
in the reference precipitation field are two strong cyclonic rain bands in the
south-eastern quadrant of the domain as well as strong convection in the
north-eastern quadrant. The ERA5 estimates capture the general cyclonic
structure and the precipitation in the north-eastern part of the scene but lack
finer convective structures. The IMERG GMI precipitation estimates yield the
best agreement with the reference estimates capturing the principal rain bands
as well as the embedded convection. The IMERG IR precipitation estimates, on the
other hand, fail to reproduce the structure of the cyclone. Although the
estimates contain substantial small-scale variability, it is unrelated to any of
the fine-scale structures in the reference data. The single-step GPROF-IR
retrievals reproduce the large-scale precipitation structures but also lack any
of the finer embedded precipitation structures. The inclusion of temporal
information yields notable and visible improvements with the retrieval being
clearly able to reproduce more of the embedded precipitation features and the
negative bias being reduced from -38 \% for the GPROF-IR (CMB, 1) retrieval to
-14 \% for the GPROF-IR (GMI, 5) retrieval.
...
over land. To complement the assessment, we therefore evaluate the GPROF-IR
...
uses GPROF-NN XPR retrieval to fill in missing light precipitation in the 2BCMB
dataset over oceans. This has been shown to reduce systematic underestimation of
high-latitude and frozen precipitation. However, the validation in Pfreundchuh
(2026) was limited a GMI overpasses over the OceanRAIN vessels. The continuous
...
reinforces the findings from the initial validation in Pfreundschuh (2026).
...
shows results for all matchups, high-latitudes (> 45 degree), and with and for
...
The above validation results show that GPROF-IR (GMI, 3) configuration is close
to GPROF-IR (GMI, 5) in terms of accuracy. Since it only needs observations from
two additional timesteps rather than the four required for the GPROF-IR (GMI, 5)
configuration, it can be more applied more flexibly and is therefore chosen as
the recommended operational configuration for the GPROF-IR retrieval. Below we
therefore characterize global precipitation climatologies of the GPROF-IR (GMI,
3) configuration derived from one year of retrievals from the year 2022. To
ensure homogeneous sampling across the datasets, we extract precipitation
estimates from GPROF-IR, ERA5, and GPCP matching the sampling of GPROF V08 GMI
estimates.
...
around 40 degree but fall below the ERA5 estimates poleward of 50 degree. Over
...
ERA5 being substantially higher. However, these differences may be enhanced due
to the smaller sample size due to lower land coverage at these latitudes.
