...
The testing split of the dataset comprises one year of GMI overpasses over
...
\citep{smith_multi-radar_2016}) products; the Korea data is derived from
gauge-corrected precipitation estimates provided by the Korean Meteorological
Agency \citep{ryu_radar_2025}. To accommodate for the testing resolution of 0.036 degree used by the
...
\begin{figure}[h]
 \centerline{\includegraphics[width=37pc]{figures/figure12.png}}
 \caption{
...
estimates during GMI overpasses over CONUS and continental Europe. Given the
limited information content of single-channel IR observations, the ability of
GPROF-IR to provide more accurate precipitation estimates than conventional PMW
retrievals for continental precipitation constitutes a major advancement for
global precipitation monitoring. It should be noted here that this holds only
conventional PMW retrieval since the GPROF-NN algorithm that will be used for the next
version of operational GPM products will notably improve the retrieval errors and
effective resolution of the GPM PMW retrievals. Validation against independent precipitation
estimates over the Korean peninsula and the global oceans showed that the
accuracy approaches but remains below that of conventional PMW retrievals over
water surfaces and precipitation regimes dominated by shallower precipitation
systems. Nonetheless, the GPROF-IR retrieval yields notable improvements over
conventional IR retrievals, substantially reducing mean absolute and mean square
error and doubling the linear correlation coefficient across a variety of
precipitation regimes.

Our experiments show that using GPROF V08 GMI instead of 2BCMB as
reference data leads to higher accuracy when compared to independent
precipitation estimates. While the GPROF V08 GMI retrieval uses the 2BCMB
dataset as reference data, it augments these precipitation estimates with
MRMS-based estimates over snow and light-precipitation estimates from the
GPROF-NN XPR \citep{pfreundschuh_xpr_2026} retrieval over ocean. Since the improved
accuracy is observed even in the evaluation against continental precipitation
where snow tends to have a small impact, the most likely explanation for the
observed improvements is the wider coverage and larger amount of the GPROF V08
GMI based reference data. Additionally, the validation against shipborne
distrometer measurements has shown that the GPROF V08-based retrieval reduces systematic
underestimation of high latitude as well as mixed and frozen precipitation
compared to the retrieval based on 2BCMB estimates. This is further evidence
that the GPROF-NN XPR retrieval is effective at improving the representation of
light and frozen precipitation in global satellite-based precipitation
estimates.

Furthermore, the our results show moderate but robust accuracy improvements when
temporal information is integrated into the retrieval. Analysis of retrieval
results of Typhoon Khanun suggests that the temporal information helps to
constrain precipitation features with weak radiometric signatures in cloud-top
temperatures such as those embedded into large-scale precipitation features.

\subsubsection{Consequences for ML Precipitation Retrieval Design}

GPROF-IR is the first single-channel IR retrieval that is able to
rival conventional PMW retrievals over continental landmasses
and demonstrate substantial, robust improvements over
conventional IR precipitation estimates. To achieve this, GPROF-IR
uses the IR observations at their native resolution since fine-scale cloud-top
structures may provide information on the underlying cloud and precipitation processes.
The positive impact of maximizing the resolution of the
input observations has also been found in \citet{pfreundschuh_improved_2022}.
Additionally, GPROF-IR leverages temporal information in the input data. This is
achieved by stacking sequential IR observations and letting the CNN model
learn to extract the temporal information that can help constrain surface
precipitation estimates. During training we noted that the multi-step retrieval do
not achieve the same accuracy as single-step retrievals when trained from scratch
with the same compute budget. We therefore initialized the multi-step
retrievals using the weights of the trained single-step retrieval. The
inclusion of  additional time steps only increases the number of input channels,
so that only the weight matrix of the first convolutional operation in the network
increases in size. All other parameters can be directly copied from the single or
three-step configurations. While we have not tested whether the multi-step
models trained from scratch eventually reach the performance, the pre-training
strategy was found to be an effective way of training multi-timestep satellite retrievals.

A notable difference to previously proposed IR-only precipitation retrievals is
that GPROF-IR uses reference data from DPR and GMI. Due to their empirical
nature, neural-network-based retrievals are unlikely to yield more accurate precipitation
estimates than the data they are trained on -- although they may potentially reduce random
errors in the reference data. Since  retrievals will generally be
less accurate than there reference data, using ERA5 or IMERG is only a
defensible design choice for applications requiring data that is more recent than three months
after which the more accurate ERA5 and IMERG final product will be available.
Additionally, GPROF-IR uses a training dataset that is relatively large compared
to other ML IR precipitation retrievals. The comparison of the GMI- and
DPR-based GPROF-IR configurations indicates that the increased amount of
training data may be responsible for the improved accuracy of the GMI-based
configuration. If this is the case, then further increasing the size of the training data
could be an opportunity to further improve IR-based precipitation retrievals.

The implementation of GPROF-IR primarily focused on maximizing the amount and
quality of the training data as well as the information content of the retrieval
input data. We have neither exhaustively tried to opitmize the model
architecture or the loss function but limited experimentation suggested that
they have a minor impact on the accuracy.
GPROF-IR is trained using a quantile loss function but the prinicipal output is
the mean of the posterior CDF, which is approximated using the 32 predicted
quantiles. The posterior mean will approximate the output that would be obtained
were the model trained using a MSE loss. However, the probabilistic nature of
the outputs has the advantage of simultaneously allowing for the calculation of
exceedance probabilities for precipitation thresholds that can be chosen during
inference time. As evident from the analysis of the precipitation detection
skill (Fig.~\ref{fig:precipitation_detection} and
Fig.~\ref{fig:precision_recall}), the exceedance thresholds yield better skill
for both precipitation detection and heavy precipitation detection. This
highlights the importance taking into the account the statistical
characteristics of different retrieval outputs when they are used to investigate
different aspects of precipitation. The posterior mean will tend to be unbiased
and minize the mean-squared-error but will yield suboptimal skill in detecting
the exceedance of specific precipitation rates. The posterior mean is provided
as the main retrieval output of GPROF-IR since we consider bias-freedom of the
precipitation climatology a desireable property for precipitation retrievals .
However, our results highlight that applications requiring different properties
(such as the representation of extremes) may benefit from using other
estimators. Future datasets should thus consider whether providing additional statistics
...
Fig.~\ref{fig:evolution} showing the GPROF-IR (GMI, 3) retrieval results
together with the GPROV V08 PMW precipitation estimates that will be used in
IMERG V08 and the current IR precipitation estimates from IMERG V07. In contrast
to the current IR precipitation estimates, GPROF-IR produces precipitation
estimates that are consistent with PMW precipitation estimates. This suggests
that the GPROF-IR retrieval will not only help to improve the overall accuracy
of IMERG V08 but also improve the representation of the temporal evolution of
precipitation systems.



\begin{figure}[h]
 \centerline{\includegraphics[width=37pc]{figures/figure15.pdf}}
 \caption{
...
are openly available to the scientific community \citep{pfreundschuh_gprofir_2026}. Since the retrieval uses only
IR observations as input, it can be easily run on the CPC gridded IR
observations. The quasi-global availability and high accuracy of the retrieval
also provides a strong baseline for the development of future precipitation
retrievals.
